...
export default function LessonContent({ contents }: LessonContentProps) {
  const { title, updateDate, description_lesson, links = [], tags = [], progress = 0 } = contents;
  ...
  return (
    <div className="w-full mx-auto bg-gradient-to-b from-gray-900 to-blue-900 dark:from-gray-950 dark:to-black p-6 sm:p-8 rounded-3xl shadow-lg mt-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-6">
        ...
        <button title="Add a new note to this lesson" className="relative group flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-orange-400 to-orange-500 text-white text-sm font-medium rounded-lg hover:from-orange-500 hover:to-orange-600 transition-all duration-200 transform hover:scale-105 shadow-sm">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {isResourcesOpen && (
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {links.map((link, index) => (
                <li key={index} className="flex items-center gap-2 bg-gray-700/30 rounded-lg p-3">
