  ...
  const renderHeader = () => (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-6">
      ...
      <button
        ...
        className="relative group flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-lime-400 to-emerald-500 text-white text-sm font-medium rounded-lg hover:from-lime-500 hover:to-emerald-600 transition-all duration-200 transform hover:scale-105 shadow-sm"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="w-full mx-auto bg-gradient-to-b from-slate-950 to-gray-900 p-8 sm:p-8 rounded-3xl shadow-lg mt-6">
        ...
        <div className="text-center">
          <div className="flex justify-center mb-4">
            <svg
              className="w-8 h-8 text-lime-400 animate-spin"
              fill="none"
              viewBox="0 0 24 24"
            >
              <circle
          ...
          <div className="mt-6 animate-pulse">
            ...
            <div className="h-4 bg-slate-700/50 rounded w-1/2 mx-auto mb-4"></div>
    ...
    <div className="w-full mx-auto bg-gradient-to-b from-slate-950 to-gray-900 p-8 sm:p-8 rounded-3xl shadow-lg mt-6">
      ...
      {progress > 0 && (
        <div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden mb-6">
          <div className="h-full bg-lime-400" style={{ width: `${progress}%` }}></div>
        </div>
      )}

      <p className="text-slate-200 text-sm sm:text-base leading-7 max-h-60 overflow-y-auto scrollbar-thin scrollbar-thumb-lime-400 scrollbar-track-slate-800 mb-8 text-left">
        {description_lesson}
      </p>
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-6">
          {tags.map((tag, index) => (
            <CustomBadge key={index} text={tag} type="tag" className="bg-lime-500/10 border border-lime-400 text-lime-300" />
          ))}
        </div>
      )}
      ...
      {links.length > 0 && (
        <div className="bg-slate-800/50 rounded-xl p-5">
          <button
            ...
            className="flex items-center gap-2 text-sm sm:text-base font-medium text-slate-200 mb-3"
          >
            Resources
            <svg
          ...
          {isResourcesOpen && (
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {links.map((link, index) => (
                <li key={index} className="flex items-center gap-2 bg-slate-700/30 rounded-lg p-3">
                  ...
                  <a
                    ...
                    className="flex items-center gap-2 text-sm sm:text-base text-lime-400 hover:text-lime-300 transition-colors duration-200 group"
                  >
                    <svg
                      className="w-4 h-4 text-slate-400 group-hover:text-lime-300 transform group-hover:scale-110 transition-transform duration-200"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <span className="underline underline-offset-4 decoration-lime-400/30 group-hover:decoration-lime-300/50">
                      {link.text}
                    </span>
                  </a>
                </li>
