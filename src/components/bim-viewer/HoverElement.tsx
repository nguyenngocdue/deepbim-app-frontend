...
const ModelIfc: React.FC<ModelIfcProps> = ({ sectionActive, coordinateSyssActive, selectedFile, onFileSelect }) => {
    ...
    useEffect(() => {
        if (!ifcContainerRef.current || !worldRef.current || !modelRef.current) return;
        ...
        const handleMouseMove = (event: MouseEvent) => {
           ...
           if (intersects.length > 0) {
               ...
               highlightHoveredElement(intersection.object as THREE.Mesh);
           } else {
    ...
    const highlightHoveredElement = (mesh: THREE.Mesh) => {
        if (!worldRef.current || !mesh) return;

        // Remove previous hover highlight
        removeHoverHighlight();

        // Change the material of the hovered mesh
        const originalMaterial = mesh.material;
        const highlightMaterial = new THREE.MeshBasicMaterial({
            color: 0xff0000, // Màu đỏ cho hover
        ...
        // Keep track of the hovered mesh
        hoveredMeshRef.current = mesh;
    };

    const removeHoverHighlight = () => {
        if (!worldRef.current || !hoveredMeshRef.current) return;

        // Restore the original material
        const originalMaterial = hoveredMeshRef.current?.userData.originalMaterial;
        if (originalMaterial) {
            hoveredMeshRef.current.material = originalMaterial;
        }
