...
const ModelIfc: React.FC<ModelIfcProps> = ({ sectionActive, coordinateSyssActive, selectedFile, onFileSelect }) => {
    ...
    const highlightSelectedFace = (intersection: THREE.Intersection) => {
        if (!worldRef.current || !intersection.face) return;
    
        // Xóa highlight cũ nếu có
        worldRef.current.scene.three.traverse((child) => {
            if (child.name === "highlightMesh") {
                worldRef.current?.scene.three.remove(child);
            }
        });
    
        // Tạo material cho highlight
        ...
        // Tính toán kích thước của mặt phẳng dựa trên geometry của object bị intersect
        const geometry = intersection.object.geometry as THREE.BufferGeometry;
        const positionAttribute = geometry.attributes.position;
        const face = intersection.face;
    
        // Lấy các đỉnh của face
        ...
        // Tính toán kích thước của mặt phẳng
        ...
        // Tạo mesh để highlight
        const highlightMesh = new THREE.Mesh(planeGeometry, highlightMaterial);
        highlightMesh.name = "highlightMesh"; // Đặt tên để dễ dàng tìm và xóa sau này
    
        // Định vị và định hướng highlightMesh
        highlightMesh.position.copy(intersection.point);
        highlightMesh.lookAt(intersection.face.normal.clone().add(intersection.point));
    
        // Thêm highlightMesh vào scene
        worldRef.current.scene.three.add(highlightMesh);
    };
    ...
    useEffect(() => {
        ...
        const handleCanvasClick = (event: MouseEvent) => {
            ...
            if (intersects.length > 0) {
                ...
                // Handle the selected face (e.g., highlight it)
                highlightSelectedFace(intersects[0]);
            }
        };
